...
const waitForEvent = (onCheck, delay, onSuccess, onFailure, onWait) => {
  ...
  .then((returnValue) => {
    if (returnValue !== undefined) {
      ...
      setTimeout(waitForEvent, delay, onCheck, delay, onSuccess, onFailure, onWait);
    }
  })
  .catch(onFailure);
}

const createEnvironment = async (instanceType, netId, subnetId, autoShutdown)  => {
  const cloud9 = new AWS.Cloud9();
  const { environmentId } = await cloud9.createEnvironmentEC2({
...
const shareEnvironment = async (environmentId, email) => {
  ...
  const userArn = Arn.replace(/-Admins\//, '-PowerUsers/').replace(/[^\/]+$/, email);
  if (userArn != Arn) {
    await cloud9.createEnvironmentMembership({
...
const getInstanceStatus = async (instanceId)  => {
  const ec2 = new AWS.EC2();
  try {
    const { InstanceStatuses } = await ec2.describeInstanceStatus({
...
const monitorCommand = (instanceId, commandId, waitCallback) => {
  return new Promise((resolve, reject) => {
    const checkCallback = async () => {
      const ssm = new AWS.SSM();
      const { Status, StatusDetails } = await ssm.getCommandInvocation({ InstanceId: instanceId, CommandId: commandId }).promise();
      switch (Status)  {
        case "Pending":
        case "InProgress":
        case "Delayed":
...
const waitForVolume = (volumeId, waitCallback) => {
  return new Promise((resolve, reject) => {
    const checkCallback = async () => {
      const ec2 = new AWS.EC2();
      const response = await ec2.describeVolumesModifications({
        VolumeIds: [volumeId],
        Filters: [
          {
...
const resizeEbsVolume = async (volumeId, diskSize, waitCallback) => {
  const ec2 = new AWS.EC2();
  const { VolumeModification } = await ec2.modifyVolume({ VolumeId: volumeId, Size: diskSize }).promise();
...
const resizeInstanceDisk = async (instanceId, diskSize, waitCallback) => {
  ...
  const volumeId = response.Reservations[0].Instances[0].BlockDeviceMappings[0].Ebs.VolumeId;
