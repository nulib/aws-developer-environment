...
/tmp/aws/install
rm -rf /tmp/aws
sh -c "$(curl -Ls {init_source}/grow_ebs_volume.sh)"
mv ~ec2-user/.c9/stop-if-inactive.sh ~ec2-user/.c9/stop-if-inactive.sh-SAVE
curl -sLo ~ec2-user/.c9/stop-if-inactive.sh {init_source}/stop-if-inactive.sh
...
as_user curl -Ls {init_source}/zshrc >> ~ec2-user/.zshrc
